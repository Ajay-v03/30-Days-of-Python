## Exception Handling
'''Python uses _try_ and _except_ to handle errors gracefully. 
...
The cause of an exception is often external to the program itself. An example of exceptions could be an incorrect input, wrong file name, unable to find a file, a malfunctioning IO device. 
Graceful handling of errors prevents our applications from crashing.

We have covered the different Python _error_ types in the previous section. 
If we use _try_ and _except_ in our program, then it will not raise errors in those blocks.
'''
try:
...
# But without any changes, the second block, _except_, will be executed.
...
dct = {'name':'Ajay', 'country':'India', 'city':'Hissar', 'age':250}
print(unpacking_person_info(**dct)) # Ajay lives in India, Hissar. He is 250 years old.
...
print(packing_person_info(name="Ajay",
      country="India", city="Hisar", age=250))

name = Ajay
country = India
city = Hisar
age = 250
{'name': 'Ajay', 'country': 'India', 'city': 'Hisar', 'age': 250}
